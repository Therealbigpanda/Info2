 ...
 *
 * The booleans are stored in an integer. Starting from the less significant bit up to the index
 * {@link Selection#size()} the respective boolean is true iff the bit is 1.
 *
...
public class Selection {
    ...
    public void setSize(int size) {
        if (size > 32) {
            throw new IllegalArgumentException("a maximum of 32 bits is supported only");
        }
        if (size < 0) {
            throw new IllegalArgumentException("the size must be greater or equal to zero");
        }
        this.size = size;
    }
    ...
    public int sum(ArrayList<Integer> addends) {
        if (addends.size() != size()) {
            throw new IllegalArgumentException("Not enough or too many addends");
        }
        int sum = 0;
